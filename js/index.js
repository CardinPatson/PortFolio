...
// document.getElementById("wrapper__popup").addEventListener("click", closeProof);
function init() {
...
function showProuf(img) {
    ...
    if (popup.style.display === "block") {
      ...
      console.log(document.querySelector(".hackathon__odoo"));
...
function showProofImage(imgList) {
    ...
    let imageDom = imgList.map(
        (image) =>
            `<div class="carousel__cell__image" style='position:absolute; left:0px'><img src='img/preuve/${image}' alt='image proof' /></div>`
    );
    ...
    // );
    imageDom[0] = `<div class="carousel__cell__image is-selected" style='position:absolute; left:0px'><img src='img/preuve/${imgList[0]}' alt='image proof' /></div>`;
...
function closeProof() {
    let imageSlider = document.querySelector("#wrapper__popup");
    let imageCarroussel = document.querySelector(".activite__proof__image");
